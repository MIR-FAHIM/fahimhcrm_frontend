...
const EmployeeAttendanceReport = () => {
  ...
  const handleEmployeeSelect = (employeeId) => {
    ...
    const next = employees.find((e) => String(e.id) === String(employeeId));
    setSelectedEmployee(next || null);
  };

  const getEmployeeLabelById = (employeeId) => {
    const found = employees.find((e) => String(e.id) === String(employeeId));
    return found?.name || "Select employee";
  };
      ...
      {isMobile ? (
        ...
        >
          ...
          <FormControl size="small" fullWidth>
            <InputLabel sx={{ color: colors.gray[400] }}>Employee</InputLabel>
            <Select
              label="Employee"
              ...
              renderValue={(selected) =>
                selected ? getEmployeeLabelById(selected) : "Select employee"
              }
              sx={{
              ...
              >
                ...
                <Box display="flex" justifyContent="flex-end" mt={1.5}>
                  <Button
                    ...
                    sx={{ textTransform: "none", color: colors.gray[100] }}
                  >
                    Reset filters
                  </Button>
                </Box>
              ...
              {attLoading ? (
                ...
                <Box display="flex" flexDirection="column" gap={2}>
                  {filteredAttendances.map((row, idx) => {
                    ...
                    const punctualityChip = !a?.check_in_time ? (
                      <Chip
                        label="N/A"
                        size="small"
                        sx={{ bgcolor: colors.gray[700], color: colors.gray[100], borderRadius: "6px" }}
                      />
                    ) : a?.is_late ? (
                      <Chip
                        label="Late"
                    ...
                    const renderStatusOrNA = (value) =>
                      a ? renderStatusYesNo(value) : (
                    ...
                    return (
                      <Paper
                        ...
                        sx={{
                          ...
                          borderRadius: 2,
                          border: `1px solid ${colors.gray[800]}`,
                          bgcolor: weekend ? colors.primary[800] : theme.palette.background.paper,
                        }}
                      >
                        <Box display="flex" alignItems="center" justifyContent="space-between" mb={1}>
                          <Box>
                            <Typography variant="subtitle2" fontWeight={700} sx={{ color: colors.gray[100] }}>
                              {row.date}
                            </Typography>
                            <Typography variant="caption" sx={{ color: colors.gray[400] }}>
                              {row.weekday}
                        ...
                        <Box display="grid" gridTemplateColumns="1fr 1fr" gap={1.5}>
                          ...
                          <Box>
                            ...
                            {renderStatusOrNA(a?.is_early_leave)}
                          </Box>
                          <Box>
                            <Typography variant="caption" color="text.secondary">
                              WFH
                            </Typography>
                            {renderStatusOrNA(a?.is_work_from_home)}
                          </Box>
                          <Box>
                            <Typography variant="caption" color="text.secondary">
                              Field
                            </Typography>
                            {renderStatusOrNA(a?.from_field)}
                          </Box>
                          <Box>
                            <Typography variant="caption" color="text.secondary">
                              Total
