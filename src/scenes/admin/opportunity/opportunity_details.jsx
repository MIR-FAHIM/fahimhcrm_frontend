...
import { getOpportunityDetail } from "../../../api/controller/admin_controller/opportunity_controller";
...
export default function OpportunityDetailsPage() {
  ...
  const [concernPersons, setConcernPersons] = useState({ prospect_id: 1, assign_to_ids: [] });
  ...
  const removeAssignedPerson = async (empId) => {
    await removeAssignPerson({ prospect_id: prospectID, employee_id: empId });
  ...
  const addMultipleConernPersons = async () => {
    const payload = {
      ...
      employees: concernPersons.assign_to_ids.map((id) => ({ employee_id: id, is_active: 0, notify: 0 })),
    };
  ...
  // mode-aware activity colors
  const activityColors = {
    general: theme.palette.mode === "dark" ? colors.orangeAccent[700] : colors.orangeAccent[900],
    task: theme.palette.mode === "dark" ? colors.blueAccent[700] : colors.blueAccent[200],
    call: theme.palette.mode === "dark" ? colors.redAccent[700] : colors.redAccent[900],
    email: theme.palette.mode === "dark" ? colors.purpleAccent[700] : colors.purpleAccent[900],
    whatsapp: theme.palette.mode === "dark" ? colors.greenAccent[700] : colors.greenAccent[900],
    visit: theme.palette.mode === "dark" ? colors.purpleAccent[800] : colors.purpleAccent[100],
    message: theme.palette.mode === "dark" ? colors.gray[800] : colors.gray[900],
    meeting: theme.palette.mode === "dark" ? colors.orangeAccent[600] : colors.orangeAccent[800],
  };
  ...
  const addLogActivity = async (activity_type) => {
    try {
      ...
      if (response.status === true) {
        ...
        if (logActivityRes.status === true) setLogActivityList(logActivityRes.data);
  ...
  const getOpportunityDetails = async () => {
    try {
      ...
      if (detailsOpporRes.status === "success") {
        ...
        const pID = opportunityData.prospect_id;
        setOpportunity(opportunityData);
        setProspectID(pID);

        const [detailsRes, stagesRes, stagesResByLog, assignedPersonsRes, contactPersonRes, logActivityRes] =
          await Promise.all([
            getProspectDetails(pID),
            getProspectAllStatus(),
            getProspectStagesByLog({ prospect_id: pID }),
            getAssignedPersonsProspect(pID),
            getContactPersonProspect(pID),
            getAllLogActivityOfProspect(pID),
          ]);
    ...
    <Box sx={{ p: 3, bgcolor: theme.palette.background.default, minHeight: "100vh" }}>
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, gap: 3 }}>
        ...
        <Box sx={{ width: { xs: "100%", md: "25%" } }}>
          <Paper sx={{ p: 3, bgcolor: theme.palette.background.paper, borderRadius: 3, boxShadow: 3 }}>
              ...
              {isEditing ? (
                <>
                  <TextField
                    ...
                    variant="standard"
                    size="small"
                    sx={{ minWidth: 200, "& input": { fontSize: "1rem", color: colors.blueAccent[100] } }}
                  />
                  <IconButton onClick={() => updateProspectInfo({ prospect_id: id, prospect_name: text })} size="small" color="primary" sx={{ ml: 0.5 }}>
                    <SaveIcon fontSize="small" />
                  </IconButton>
                </>
              ) : (
                <>
                  <Typography variant="h6" sx={{ color: colors.blueAccent[100], fontWeight: 600 }}>
                    {details.prospect_name}
                  </Typography>
                  <IconButton
                    size="small"
                    onClick={() => { setText(details.prospect_name); setIsEditing(true); }}
                    sx={{ ml: 0.5, p: 0.5, color: colors.gray[400], "&:hover": { bgcolor: "transparent", color: colors.gray[200] } }}
                  >
            ...
            <Typography variant="body2" sx={{ color: colors.gray[300], mb: 0.5 }}>Opportunity ID: #{id}</Typography>
            <Typography variant="body2" sx={{ color: colors.gray[300], mb: 1 }}>Lead ID: #{details.id}</Typography>
            ...
            {(details.is_opportunity === 0
              ...
              : ["Assigned To", "Details", "Attached Files", "Leads", "Quotations", "Orders"]).map((label, i) => (
              <Accordion key={i} sx={{ mt: 2, bgcolor: theme.palette.background.paper }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
                          <FormControl fullWidth sx={{ flex: 1, minWidth: 250 }}>
                            ...
                            <Select
                              ...
                              renderValue={(selected) => employees.filter((e) => selected.includes(e.id)).map((e) => e.name).join(", ")}
                            >
                          ...
                          <Button variant="contained" color="primary" sx={{ whiteSpace: "nowrap" }} onClick={addMultipleConernPersons}>
                            Add
                          </Button>
                        </Box>
                      </Box>
                      ...
                      <Box sx={{ maxHeight: 250, overflowY: "auto", mt: 1, pr: 1 }}>
                        {assignedPersons && assignedPersons.length > 0 ? (
                          assignedPersons.map((person) => (
                            <Box key={person.id} sx={{ mb: 1.5, p: 1.5, bgcolor: colors.gray[900], borderRadius: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <Box>
                                <Typography variant="body1" sx={{ color: colors.gray[100], fontWeight: 700 }}>{person.employee.name}</Typography>
                                <Typography variant="body2" sx={{ color: colors.gray[300] }}>📱 {person.employee.phone}</Typography>
                                <Typography variant="body2" sx={{ color: colors.blueAccent[300] }}>✉️ {person.employee.email}</Typography>
                              </Box>
                              <IconButton color="error" onClick={() => removeAssignedPerson(person.employee.id)}>
                                <DeleteIcon />
                              </IconButton>
                            </Box>
                          ))
                        ) : (
                          <Typography variant="body2" sx={{ color: colors.gray[400], mt: 1 }}>No concerned persons assigned.</Typography>
                        )}
                      </Box>
                    </Box>
                  ) : (
        ...
        <Box sx={{ width: { xs: "100%", md: "75%" } }}>
          <Paper sx={{ p: 3, mb: 3, bgcolor: theme.palette.background.paper, borderRadius: 3, boxShadow: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 3, flexWrap: "wrap", mb: 2, justifyContent: "space-between" }}>
              {details.created_at && (
              ...
              <Box sx={{ minWidth: 250 }}>
                <TextField select fullWidth name="stage_id" label="Change Stage" value={form.stage_id} onChange={(e) => handleChange(e.target.value)}
                  sx={{
                    "& .MuiOutlinedInput-root": { bgcolor: theme.palette.background.default, color: colors.gray[100] },
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, p: 1, bgcolor: colors.gray[900], borderRadius: 2, boxShadow: 1 }}>
            ...
            <Box sx={{ overflowX: "auto", py: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, minWidth: "600px" }}>
                {stagesByLog.map((stage, index) => {
                  const isCompleted = stage.id < prospectStageId;
                  const isCurrent = stage.id === prospectStageId;
                  const tooltipTitle = stage.last_updated_at ? `Last Updated: ${stage.last_updated_at}\nBy ${stage.changed_by_name}` : "Not yet visited";
                  return (
                    <Box key={stage.id} sx={{ display: "flex", alignItems: "center", flexDirection: "column" }}>
                      <Tooltip title={<Typography sx={{ whiteSpace: "pre-line" }}>{tooltipTitle}</Typography>} arrow>
                        <Chip label={stage.stage_name} color={isCurrent || isCompleted ? "success" : "default"} variant={isCurrent ? "filled" : "outlined"} sx={{ cursor: "pointer" }} />
                      </Tooltip>
                      ...
                      {index !== stagesByLog.length - 1 && (
                        <Box sx={{ width: 20, height: 2, bgcolor: colors.gray[700], mx: 1 }} />
                      )}
                    </Box>
                  );
                })}
            ...
            {/* Tabs */}
            <Tabs value={tabValue} onChange={(_e, v) => setTabValue(v)} sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
            ...
            {tabValue === 0 ? (
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <TextField multiline rows={3} label="Log Description" variant="outlined" fullWidth value={logNote} onChange={(e) => setLogNote(e.target.value)}
                  sx={{ "& .MuiOutlinedInput-root": { bgcolor: theme.palette.background.default, color: colors.gray[100] } }}
                />
                ...
                <Box sx={{ mt: 3 }}>
                  <Typography variant="subtitle1" sx={{ color: colors.gray[100], mb: 2 }}>Templates</Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={6}>
                      <Typography variant="subtitle2" sx={{ color: colors.gray[100], mb: 1 }}>Favorite Templates</Typography>
                      <Box sx={{ height: 175, overflow: "auto", pr: 1 }}>
                        {["Called the client – no answer","Spoke over WhatsApp – client asked for brochure","Sent follow-up email regarding product demo","Visited office – client was not available","Client requested a call back via WhatsApp","Had a brief call – client asked for pricing details","Emailed updated proposal","Visited client – gave live demo","Shared product catalog on WhatsApp","Follow-up call made – spoke with assistant","Client responded on email, requested more info","Left voicemail – awaiting response","Client said they will confirm demo date via WhatsApp","Sent reminder email for scheduled meeting","Dropped by client’s office – they were in a meeting"].map((text, i) => (
                          <Paper key={i} onClick={() => setLogNote(text)} sx={{ p: 2, mt: 1, bgcolor: colors.gray[900], cursor: "pointer" }}>
                            <Typography variant="body2" sx={{ color: colors.gray[100] }}>{text}</Typography>
                          </Paper>
                        ))}
                      </Box>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <Typography variant="subtitle2" sx={{ color: colors.gray[100], mb: 1 }}>All Templates</Typography>
                      <Box sx={{ height: 175, overflow: "auto", pr: 1 }}>
                        {["Called the client – no answer","Spoke over WhatsApp – client asked for brochure","Sent follow-up email regarding product demo","Visited office – client was not available","Client requested a call back via WhatsApp","Had a brief call – client asked for pricing details","Emailed updated proposal","Visited client – gave live demo","Shared product catalog on WhatsApp","Follow-up call made – spoke with assistant","Client responded on email, requested more info","Left voicemail – awaiting response","Client said they will confirm demo date via WhatsApp","Sent reminder email for scheduled meeting","Dropped by client’s office – they were in a meeting"].map((text, i) => (
                          <Paper key={i} onClick={() => setLogNote(text)} sx={{ p: 2, mt: 1, bgcolor: colors.gray[900], cursor: "pointer" }}>
                            <Typography variant="body2" sx={{ color: colors.gray[100] }}>{text}</Typography>
